  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api-events') {
      try {
        ...
        if (!markdown) throw new Error("Não foi possível carregar a agenda oficial da WSL.");
        ...
        return new Response(JSON.stringify({ sucesso: false, mensagem: "Falha ao buscar calendário: " + err.message }), { headers: corsHeaders });
      }
    }

    // 2. Resultados da Etapa (/api-wsl)
    if (url.pathname === '/api-wsl') {
      ...
      try {
        ...
        if (!mainMarkdown) throw new Error("Não foi possível obter os dados da etapa na WSL.");
        ...
        return new Response(JSON.stringify({ sucesso: false, mensagem: "Falha na extração de baterias: " + err.message }), { headers: corsHeaders });
      }
    }
